...
@section('content')
    <div class="container pt-3">
        ...
        <form action="{{ route('admin.products.store') }}" class="row g-3" method="POST" enctype="multipart/form-data" onsubmit="return(validate())">
            @csrf()
            ...
            <div class="col-12">
                ...
                {{-- , $name?->email) }} = stampare il valore di email --}}
                {{-- , $name?->email) }} = "?" se la variabile $name non è definito assegna "null"  --}}
                <input step="0.01" type="number" class="form-control @error('price') is-invalid @enderror" value="{{ old('price') }}" id="inputprice" name="price">
            ...
            <div class="form-check @error('visible') is-invalid @enderror">
                <input class="form-check-input type-radios" type="radio" name="visible" id="flexRadioDefault1" value="1">
                <label class="form-check-label" for="flexRadioDefault1">
                    Si
                </label>
            </div>
            <div class="form-check">
                <input class="form-check-input type-radios" type="radio" name="visible" id="flexRadioDefault2" value="0">
                <label class="form-check-label" for="flexRadioDefault1">
                    No
                </label>
            </div>
            ...
            @error('visible')
                <div class="invalid_feedback">{{ $message }}</div>
            @enderror
            <button class="btn btn-primary" type="submit">Create your products</button>
        </form>
    </div>

...
@section('content')
    <div class="container pt-3">
        <h1 class="m-5 text-center">Inserisci i dati per registrare il tuo ristorante!</h1>
        ...
        <form action="{{ route('admin.restaurants.store') }}" class="row g-3" method="POST" enctype="multipart/form-data"
            ...
            <div class="col-12">
                <label for="inputTitle" class="form-label">Numero di telefono<span class="text-danger">*</span></label>
                {{-- value="{{ old('email'= ottenere il valore precedentemente inviato --}}
                {{-- , $name?->email) }} = stampare il valore di email --}}
                {{-- , $name?->email) }} = "?" se la variabile $name non è definito assegna "null"  --}}
            ...
            <div class="col-12">
                <label for="inputTitle" class="form-label">Nome dell'attività <span class="text-danger">*</span></label>
                {{-- value="{{ old('email'= ottenere il valore precedentemente inviato --}}
                {{-- , $name?->email) }} = stampare il valore di email --}}
                {{-- , $name?->email) }} = "?" se la variabile $name non è definito assegna "null"  --}}
            ...
            <div class="col-12">
                <label for="inputTitle" class="form-label">Immagine Ristorante <span class="text-danger">*</span></label>
                {{-- value="{{ old('email'= ottenere il valore precedentemente inviato --}}
                {{-- , $name?->email) }} = stampare il valore di email --}}
                {{-- , $name?->email) }} = "?" se la variabile $name non è definito assegna "null"  --}}
            ...
            <div class="col-12">
                <label for="inputTitle" class="form-label">Indirizzo <span class="text-danger">*</span></label>
                {{-- value="{{ old('email'= ottenere il valore precedentemente inviato --}}
                {{-- , $name?->email) }} = stampare il valore di email --}}
                {{-- , $name?->email) }} = "?" se la variabile $name non è definito assegna "null"  --}}
                <input type="text" class="form-control @error('address') is-invalid @enderror"
                    value="{{ old('address') }}" id="inputaddress" name="address" placeholder="Esempio: Via Brombeis n°23">
                <div id="error-address" class="invalid_feedback"></div>
                @error('address')
                    <div class="invalid_feedback">{{ $message }}</div>
                @enderror
            </div>
            <label for="inputTitle" class="form-label">Tipologia <span class="text-danger">*</span></label>

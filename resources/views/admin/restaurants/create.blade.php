...
@section('content')
    <div class="container pt-3">
        ...
        <form action="{{ route('admin.restaurants.store') }}" class="row g-3" method="POST" enctype="multipart/form-data" onsubmit="return(validate())">
            @csrf()
            ...
            <div class="col-12">
                ...
                {{-- , $name?->email) }} = stampare il valore di email --}}
                {{-- , $name?->email) }} = "?" se la variabile $name non è definito assegna "null"  --}}
                <input type="text" class="form-control @error('address') is-invalid @enderror" value="{{ old('address') }}" id="inputaddress" name="address" placeholder="Esempio: Via Brombeis n°23">
            ...
            <div class="d-flex gap-3">
                @foreach ($types as $type)
                    <div class="form-check form-switch  @error('types') is-invalid @enderror">
                        <input class="form-check-input type-checkbox" type="checkbox" name="types[]" role="switch" id="{{ $type->id }}" value="{{ $type->id }}">
